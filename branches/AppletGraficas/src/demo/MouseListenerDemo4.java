...
public class MouseListenerDemo4 extends ApplicationFrame
    ...
    public void chartMouseMoved(ChartMouseEvent event) {
        ...
        Vector<Comparable> vSeries = new Vector<Comparable>();
        
        if (entity != null) {
            System.out.println(
                "Mouse moved: " + x + ", " + y + ": " + entity.toString());
            //entity.setToolTipText("Hola mundo");
            XYPlot plot = (XYPlot) this.chart.getPlot();
            XYItemRenderer renderer = plot.getRenderer();
            XYDataset dataset = plot.getDataset();
            
            for(int i=0;i<dataset.getSeriesCount();i++){
            	vSeries.add(dataset.getSeriesKey(i));
            }
            	
            for (int i = 0; i < dataset.getSeriesCount(); i++) {
                renderer.setSeriesStroke(i, new BasicStroke(1.0f));
                if (vSeries.contains(dataset.getSeriesKey(i))) {
